    ...
    try:
        ...
        for col in numeric_cols:
            if col in df.columns:
                ...
                logger.debug(f"Numeric column '{col}' not found in health_records.csv, will be created as NaN if needed by functions.")
        ...
        for col in string_cols:
            if col in df.columns:
                df[col] = df[col].astype(str).fillna('Unknown')
                 ...
                 logger.debug(f"String column '{col}' not found in health_records.csv, will be created as 'Unknown' if needed by functions.")
...
def load_iot_clinic_environment_data():
    ...
    try:
        ...
        required_cols = ['timestamp', 'clinic_id', 'room_name', 'avg_co2_ppm']
        if not all(col in df.columns for col in required_cols): # pragma: no cover
            missing = [col for col in required_cols if col not in df.columns]
        ...
        for col in numeric_iot_cols:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
                 ...
                 logger.debug(f"Numeric IoT column '{col}' not found. Will be NaN if accessed.")
...
def enrich_zone_geodata_with_health_aggregates(zone_gdf, health_df, iot_df=None):
    if (health_df.empty or health_df is None) and (iot_df is None or iot_df.empty):
        logger.warning("Health records and IoT DataFrames are empty. Cannot enrich zone geodata significantly.")
        default_cols = ['avg_risk_score', 'active_tb_cases', 'active_malaria_cases', 'hiv_positive_cases',
                        ...
                        'zone_avg_co2', 'zone_avg_temp']
        zone_gdf_enriched = zone_gdf.copy() if zone_gdf is not None and not zone_gdf.empty else gpd.GeoDataFrame(crs=zone_gdf.crs if zone_gdf is not None else "EPSG:4326")
        for col in default_cols:
            if col not in zone_gdf_enriched.columns: zone_gdf_enriched[col] = 0.0
        return zone_gdf_enriched

    if zone_gdf is None or zone_gdf.empty: # pragma: no cover
        logger.warning("Zone GeoDataFrame is empty or None. Cannot enrich.")
        return zone_gdf

    logger.info(f"Enriching zone geodata ({len(zone_gdf)} zones) with health ({len(health_df if health_df is not None else [])} recs) & IoT ({len(iot_df if iot_df is not None else [])} recs).")
    ...
    if health_df is not None and not health_df.empty:
        ...
        def count_unique_patients_with_condition(condition_list):
            # Ensure 'condition' column exists and is string type for .isin()
            if 'condition' not in health_df_copy.columns: return lambda x: 0
            return lambda x: health_df_copy.loc[x.index]['condition'].astype(str).isin(condition_list).nunique() if not x.empty else 0
        ...
        zone_summary_aggregations = {
            ...
            'sti_cases': ('patient_id', count_unique_patients_with_condition([c for c in health_df_copy['condition'].unique() if str(c).startswith('STI-')])),
        ...
        # Filter aggregations for columns that actually exist in health_df_copy
        valid_aggregations = {
            key: val for key, val in zone_summary_aggregations.items() 
            if isinstance(val, tuple) and val[0] in health_df_copy.columns
        }
        if valid_aggregations:
            ...
            logger.warning("No valid columns found in health_df for aggregation.")
            
    expected_health_agg_cols = ['avg_risk_score', 'chw_visits_in_zone', 'active_tb_cases', 'active_malaria_cases',
    ...
    for col in expected_health_agg_cols:
    ...
    if iot_df is not None and not iot_df.empty:
        ...
        if 'zone_id' not in iot_df_copy.columns: # pragma: no cover
            logger.warning("zone_id missing in IoT data, cannot aggregate by zone for enrichment.")
        else:
            ...
            for iot_col_num in ['avg_co2_ppm', 'max_co2_ppm', 'avg_temp_celsius']:
                if iot_col_num in iot_df_copy.columns:
                    iot_df_copy[iot_col_num] = pd.to_numeric(iot_df_copy[iot_col_num], errors='coerce')
                else: # pragma: no cover
                    iot_df_copy[iot_col_num] = pd.NA # So mean/max can handle it

            latest_iot_zone = iot_df_copy.sort_values('timestamp').drop_duplicates(subset=['zone_id', 'room_name'], keep='last')
            iot_zone_summary = latest_iot_zone.groupby('zone_id').agg(
    ...
    enriched_gdf['total_active_key_infections'] = 0.0 # Initialize as float
    for col_inf in key_infectious_cols:
        if col_inf in enriched_gdf.columns:
            enriched_gdf['total_active_key_infections'] += pd.to_numeric(enriched_gdf[col_inf], errors='coerce').fillna(0)
        else: enriched_gdf[col_inf] = 0.0
...
def get_overall_kpis(df_health_records):
    if df_health_records.empty:
        return {"total_patients": 0, "tb_cases_active": 0, "malaria_rdt_positive_rate":0.0, "hiv_newly_diagnosed_period": 0, "pending_critical_referrals": 0}
    df_copy = df_health_records.copy()
    if 'date' not in df_copy.columns or df_copy['date'].isnull().all(): return {"total_patients": 0, "tb_cases_active": 0, "malaria_rdt_positive_rate":0.0, "hiv_newly_diagnosed_period": 0, "pending_critical_referrals": 0}
    df_copy['date'] = pd.to_datetime(df_copy['date'], errors='coerce'); df_copy.dropna(subset=['date'], inplace=True)
    if df_copy.empty: return {"total_patients": 0, "tb_cases_active": 0, "malaria_rdt_positive_rate":0.0, "hiv_newly_diagnosed_period": 0, "pending_critical_referrals": 0}
    
    latest_date = df_copy['date'].max(); period_start_date = latest_date - pd.Timedelta(days=app_config.DEFAULT_DATE_RANGE_DAYS_TREND)
    total_patients = df_copy['patient_id'].astype(str).nunique()
    tb_cases_active = df_copy[(df_copy['condition'].astype(str) == 'TB') & (df_copy['test_result'].astype(str) == 'Positive') & (df_copy['referral_status'].astype(str) != 'Completed')]['patient_id'].astype(str).nunique()
    
    malaria_rdt_tests = df_copy[df_copy['test_type'].astype(str) == 'RDT-Malaria']
    malaria_rdt_positive = malaria_rdt_tests[malaria_rdt_tests['test_result'].astype(str) == 'Positive'].shape[0]
    malaria_rdt_total = malaria_rdt_tests[~malaria_rdt_tests['test_result'].astype(str).isin(['Pending', 'N/A', 'Unknown', 'nan'])].shape[0] # Conclusive
    malaria_rdt_positive_rate = (malaria_rdt_positive / malaria_rdt_total) * 100 if malaria_rdt_total > 0 else 0.0
    
    df_copy['test_date'] = pd.to_datetime(df_copy['test_date'], errors='coerce')
    hiv_newly_diagnosed = df_copy[
        (df_copy['condition'].astype(str) == 'HIV-Positive') & 
        (df_copy['test_result'].astype(str) == 'Positive') & 
        (df_copy['test_type'].astype(str).str.contains("HIV", case=False, na=False)) & 
        (df_copy['test_date'].notna()) & (df_copy['test_date'] >= period_start_date)
    ]['patient_id'].astype(str).nunique()
    
    pending_critical_referrals = df_copy[(df_copy['referral_status'].astype(str) == 'Pending') & (df_copy['condition'].astype(str).isin(app_config.KEY_CONDITIONS_FOR_TRENDS))]['patient_id'].astype(str).nunique()
...
def get_chw_summary(df_chw_view):
    if df_chw_view.empty:
        return {"visits_today": 0, "tb_contacts_to_trace": 0, "sti_symptomatic_referrals": 0, "avg_patient_risk_visited":0.0, "patients_low_spo2_visited": 0, "patients_fever_visited": 0}
    df_copy = df_chw_view.copy()
    df_copy['chw_visit'] = pd.to_numeric(df_copy.get('chw_visit'), errors='coerce').fillna(0)
    df_copy['condition'] = df_copy.get('condition','Unknown').astype(str)
    df_copy['tb_contact_traced'] = pd.to_numeric(df_copy.get('tb_contact_traced'), errors='coerce').fillna(0)
    df_copy['ai_risk_score'] = pd.to_numeric(df_copy.get('ai_risk_score'), errors='coerce')
    df_copy['referral_status'] = df_copy.get('referral_status','Unknown').astype(str)
    df_copy['avg_spo2_pct'] = pd.to_numeric(df_copy.get('avg_spo2_pct'), errors='coerce')
    df_copy['max_skin_temp_celsius'] = pd.to_numeric(df_copy.get('max_skin_temp_celsius'), errors='coerce')

    visits_today = df_copy[df_copy['chw_visit'] == 1].shape[0]
    tb_contacts_to_trace = df_copy[(df_copy['condition'] == 'TB') & (df_copy['tb_contact_traced'] == 0)].shape[0]
    sti_symptomatic_referrals = df_copy[(df_copy['condition'].str.startswith("STI-", na=False)) & (df_copy['referral_status'] == 'Pending')].shape[0]
    
    patients_visited_today_df = df_copy[df_copy['chw_visit'] == 1]
    ...
    patients_low_spo2_visited = patients_visited_today_df[patients_visited_today_df['avg_spo2_pct'] < app_config.SPO2_LOW_THRESHOLD_PCT].shape[0] if 'avg_spo2_pct' in patients_visited_today_df else 0
    patients_fever_visited = patients_visited_today_df[patients_visited_today_df['max_skin_temp_celsius'] >= app_config.SKIN_TEMP_FEVER_THRESHOLD_C].shape[0] if 'max_skin_temp_celsius' in patients_visited_today_df else 0
...
def get_patient_alerts_for_chw(df_chw_view, risk_threshold=None):
    ...
    df_copy['ai_risk_score'] = pd.to_numeric(df_copy.get('ai_risk_score'), errors='coerce')
    for col_str in ['test_result', 'condition', 'referral_status', 'patient_id', 'zone_id']:
        df_copy[col_str] = df_copy.get(col_str, 'Unknown').astype(str).fillna('Unknown')
    df_copy['tb_contact_traced'] = pd.to_numeric(df_copy.get('tb_contact_traced'), errors='coerce').fillna(0)
    df_copy['min_spo2_pct'] = pd.to_numeric(df_copy.get('min_spo2_pct'), errors='coerce')
    df_copy['max_skin_temp_celsius'] = pd.to_numeric(df_copy.get('max_skin_temp_celsius'), errors='coerce')
    df_copy['fall_detected_today'] = pd.to_numeric(df_copy.get('fall_detected_today'), errors='coerce').fillna(0)
    ...
    def determine_reason_chw_focused_extended(row):
        if pd.notna(row.get('fall_detected_today')) and row['fall_detected_today'] > 0: return f"Fall Detected"
        if pd.notna(row.get('min_spo2_pct')) and row['min_spo2_pct'] < app_config.SPO2_LOW_THRESHOLD_PCT: return f"Low SpO2 ({row['min_spo2_pct']:.0f}%)"
        if pd.notna(row.get('max_skin_temp_celsius')) and row['max_skin_temp_celsius'] >= app_config.SKIN_TEMP_FEVER_THRESHOLD_C: return f"Fever ({row['max_skin_temp_celsius']:.1f}°C)"
        if row['condition'] == 'TB' and row['tb_contact_traced'] == 0: return "TB Contact Trace"
        if row['condition'] == 'TB' and row['referral_status'] == 'Pending': return "TB Referral Pend."
        if str(row['condition']).startswith("STI-") and row['referral_status'] == 'Pending': return f"STI Follow-up"
        if row['condition'] == 'HIV-Positive' and row['referral_status'] == 'Pending': return "HIV Linkage Care"
        if row['condition'] == 'Pneumonia' and pd.notna(row['ai_risk_score']) and row['ai_risk_score'] >= app_config.RISK_THRESHOLDS['moderate']: return f"Pneumonia (Risk: {row['ai_risk_score']:.0f})"
    ...
    alerts_df = alerts_df.sort_values(by=['ai_risk_score', 'date'], ascending=[False, False]); alerts_df.drop_duplicates(subset=['patient_id'], keep='first', inplace=True)
    output_cols = ['patient_id', 'zone_id', 'condition', 'ai_risk_score', 'alert_reason', 'referral_status', 'date', 'min_spo2_pct', 'max_skin_temp_celsius', 'fall_detected_today']
    existing_output_cols = [col for col in output_cols if col in alerts_df.columns]
    return alerts_df[existing_output_cols]


@st.cache_data(hash_funcs={pd.DataFrame: lambda df: df.to_parquet() if not df.empty and isinstance(df, pd.DataFrame) else None})
def get_clinic_summary(df_clinic_view): # Disease-focused KPIs
    if df_clinic_view.empty:
        return {"tb_sputum_positivity": 0.0, "malaria_rdt_positivity": 0.0, "sti_tests_pending":0, "hiv_tests_done_period":0, "critical_disease_supply_items": 0}

    df_copy = df_clinic_view.copy()
    for col in ['test_type', 'test_result', 'supply_item']: df_copy[col] = df_copy.get(col, 'Unknown').astype(str)
    df_copy['test_turnaround_days'] = pd.to_numeric(df_copy.get('test_turnaround_days'), errors='coerce')
    df_copy['supply_level_days'] = pd.to_numeric(df_copy.get('supply_level_days'), errors='coerce')


    tb_tests = df_copy[df_copy['test_type'].str.contains("Sputum", case=False, na=False)]
    ...
    malaria_tests = df_copy[df_copy['test_type'].str.contains("RDT-Malaria", case=False, na=False)]
    ...
    sti_test_types = app_config.CRITICAL_TESTS_PENDING # Assume these include STI tests
    sti_tests_pending = df_copy[(df_copy['test_type'].isin(sti_test_types)) & (df_copy['test_result'] == 'Pending')].shape[0]
    hiv_tests_done_period = df_copy[df_copy['test_type'].str.contains("HIV", case=False, na=False)].shape[0]
    ...
    critical_disease_supply_items = df_copy[
        (df_copy['supply_level_days'].notna()) &
        (df_copy['supply_level_days'] <= app_config.CRITICAL_SUPPLY_DAYS) &
        (df_copy['supply_item'].str.contains('|'.join(key_disease_supplies_substrings), case=False, na=False))
    ]['supply_item'].nunique()
...
def get_clinic_environmental_summary(df_iot_clinic_view):
    # ... (Function as provided in the previous "act as SME and add..." response, ensure robustness)
    if df_iot_clinic_view.empty:
        return {"avg_co2": 0, "co2_alert_rooms": 0, "avg_pm25": 0, "pm25_alert_rooms": 0, "avg_occupancy":0, "high_occupancy_alert": False, "avg_sanitizer_use_hr":0}
    df_copy = df_iot_clinic_view.copy()
    for col_iot_num in ['avg_co2_ppm', 'max_co2_ppm', 'avg_pm25', 'voc_index', 'avg_temp_celsius', 'avg_humidity_rh', 'avg_noise_db', 'waiting_room_occupancy', 'patient_throughput_per_hour', 'sanitizer_dispenses_per_hour']:
        df_copy[col_iot_num] = pd.to_numeric(df_copy.get(col_iot_num), errors='coerce')
    
    avg_co2 = df_copy['avg_co2_ppm'].mean(); avg_pm25 = df_copy['avg_pm25'].mean(); avg_occupancy = df_copy['waiting_room_occupancy'].mean(); avg_sanitizer_use_hr = df_copy['sanitizer_dispenses_per_hour'].mean()
    latest_iot_per_room = df_copy.sort_values('timestamp').drop_duplicates(subset=['clinic_id', 'room_name'], keep='last')
    co2_alert_rooms = latest_iot_per_room[latest_iot_per_room['avg_co2_ppm'] > app_config.CO2_LEVEL_ALERT_PPM].shape[0] if 'avg_co2_ppm' in latest_iot_per_room else 0
    pm25_alert_rooms = latest_iot_per_room[latest_iot_per_room['avg_pm25'] > app_config.PM25_ALERT_UGM3].shape[0] if 'avg_pm25' in latest_iot_per_room else 0
    high_occupancy_alert = (latest_iot_per_room['waiting_room_occupancy'] > app_config.TARGET_WAITING_ROOM_OCCUPANCY).any() if 'waiting_room_occupancy' in latest_iot_per_room else False
...
def get_patient_alerts_for_clinic(df_clinic_view, risk_threshold=None):
    # ... (Function as provided in the "complete corrected file" response, robust with type checks)
    if df_clinic_view.empty: return pd.DataFrame()
    df_copy = df_clinic_view.copy(); df_copy['date'] = pd.to_datetime(df_copy.get('date'), errors='coerce'); df_copy['test_date'] = pd.to_datetime(df_copy.get('test_date'), errors='coerce'); df_copy.dropna(subset=['date'], inplace=True)
    if df_copy.empty: return pd.DataFrame()
    df_copy['ai_risk_score'] = pd.to_numeric(df_copy.get('ai_risk_score'), errors='coerce')
    for col_str_ca in ['test_result', 'condition', 'referral_status', 'patient_id', 'zone_id']: df_copy[col_str_ca] = df_copy.get(col_str_ca, 'Unknown').astype(str).fillna('Unknown')
    risk_thresh = risk_threshold if risk_threshold is not None else app_config.RISK_THRESHOLDS['moderate']; latest_view_date = df_copy['date'].max(); recent_test_cutoff = latest_view_date - pd.Timedelta(days=7)
    cond1_ca = df_copy['ai_risk_score'].notna() & (df_copy['ai_risk_score'] >= risk_thresh); cond2_ca = (df_copy['test_result'] == 'Positive') & df_copy['test_date'].notna() & (df_copy['test_date'] >= recent_test_cutoff); cond3_ca = (df_copy['test_result'] == 'Pending') & (df_copy['condition'].isin(app_config.KEY_CONDITIONS_FOR_TRENDS) & df_copy['test_type'].isin(app_config.CRITICAL_TESTS_PENDING)) # More specific pending
    alert_conditions = cond1_ca | cond2_ca | cond3_ca; alerts_df = df_copy[alert_conditions].copy()
    if alerts_df.empty: return pd.DataFrame()
    def determine_reason_clinic(row):
        if pd.notna(row['ai_risk_score']) and row['ai_risk_score'] >= risk_thresh: return f"High Risk ({row['ai_risk_score']:.0f})"
        if row['test_result'] == 'Positive' and pd.notna(row['test_date']) and row['test_date'] >= recent_test_cutoff: return f"Recent Positive ({row['condition']})"
        if row['test_result'] == 'Pending' and row['condition'] in app_config.KEY_CONDITIONS_FOR_TRENDS and row['test_type'] in app_config.CRITICAL_TESTS_PENDING: return f"Pending Critical Test ({row['condition']})"
        return "Review Case"
    alerts_df['alert_reason'] = alerts_df.apply(determine_reason_clinic, axis=1); alerts_df = alerts_df.sort_values(by=['ai_risk_score', 'date'], ascending=[False, False]); alerts_df.drop_duplicates(subset=['patient_id'], keep='first', inplace=True)
    output_cols_ca = ['patient_id', 'zone_id', 'condition', 'ai_risk_score', 'test_result', 'referral_status', 'alert_reason', 'date']; existing_output_cols_ca = [col for col in output_cols_ca if col in alerts_df.columns]
...
def get_trend_data(df, value_col, date_col='date', period='D', agg_func='mean'):
    ...
    if df.empty or value_col not in df.columns or date_col not in df.columns:
        logger.warning(f"Trend: '{value_col}' - DataFrame empty or columns missing."); return pd.Series(dtype='float64').rename_axis(date_col if date_col in df.columns else 'date')
    df_copy = df.copy()
    try:
        df_copy[date_col] = pd.to_datetime(df_copy[date_col], errors='coerce')
        if agg_func in ['mean', 'sum']: df_copy[value_col] = pd.to_numeric(df_copy[value_col], errors='coerce')
        df_copy.dropna(subset=[date_col, value_col], inplace=True)
        if df_copy.empty: logger.warning(f"Trend: '{value_col}' - No valid data after NaT/NaN drop."); return pd.Series(dtype='float64').rename_axis(date_col)
    ...
    except Exception as e: logger.error(f"Trend Error for '{value_col}': {e}", exc_info=True); return pd.Series(dtype='float64').rename_axis(date_col if date_col in df.columns else 'date')
...
def get_supply_forecast_data(df_health_records, forecast_days_out=7):
    ...
    df_copy = df_health_records.copy(); df_copy['date'] = pd.to_datetime(df_copy.get('date'), errors='coerce'); df_copy['supply_item'] = df_copy.get('supply_item','Unknown').astype(str).fillna('Unknown'); df_copy['supply_level_days'] = pd.to_numeric(df_copy.get('supply_level_days'), errors='coerce')
    latest_supplies = df_copy[~df_copy['supply_item'].isin(['Unknown', 'N/A', 'nan'])].sort_values('date').drop_duplicates(subset=['supply_item'], keep='last'); latest_supplies = latest_supplies[latest_supplies['supply_level_days'].notna() & (latest_supplies['supply_level_days'] > 0)]
    ...
    for _, row in latest_supplies.iterrows():
        item = row['supply_item']; current_level_days = row['supply_level_days']; last_recorded_date = row['date']
        if pd.isna(last_recorded_date): continue 
...
def get_district_summary_kpis(enriched_zone_gdf):
    # ... (Function as provided in the "focus on TB, Malaria..." response, with disease burden KPIs)
    if enriched_zone_gdf is None or enriched_zone_gdf.empty:
        return {"avg_population_risk": 0.0, "overall_facility_coverage": 0.0, "zones_high_risk": 0, "district_tb_burden":0, "district_malaria_burden":0}
    gdf_copy = enriched_zone_gdf.copy()
    # Ensure all required columns for calculation exist and are numeric, fill with 0 if not.
    cols_to_check_numeric = ['population', 'avg_risk_score', 'facility_coverage_score', 
                             'active_tb_cases', 'active_malaria_cases', # Add other disease counts if used here
                             'avg_daily_steps_zone', 'avg_spo2_zone', 'zone_avg_co2'] # Sensor data
    for col_dist_kpi in cols_to_check_numeric: 
        if col_dist_kpi in gdf_copy.columns: gdf_copy[col_dist_kpi] = pd.to_numeric(gdf_copy[col_dist_kpi], errors='coerce').fillna(0)
        else: gdf_copy[col_dist_kpi] = 0.0
    
    total_population = gdf_copy['population'].sum()
    avg_pop_risk = (gdf_copy['avg_risk_score'] * gdf_copy['population']).sum() / total_population if total_population > 0 else 0.0
    overall_facility_coverage = (gdf_copy['facility_coverage_score'] * gdf_copy['population']).sum() / total_population if total_population > 0 else 0.0
    zones_high_risk = gdf_copy[gdf_copy['avg_risk_score'] >= app_config.RISK_THRESHOLDS['district_zone_high_risk']].shape[0]
    district_tb_burden = gdf_copy['active_tb_cases'].sum(); district_malaria_burden = gdf_copy['active_malaria_cases'].sum()
    avg_zone_co2 = gdf_copy['zone_avg_co2'].mean() if 'zone_avg_co2' in gdf_copy and gdf_copy['zone_avg_co2'].notna().any() else 0.0 # Example new KPI
