...
def hash_geodataframe(gdf):
    ...
    if not isinstance(gdf, gpd.GeoDataFrame):
        # Fallback for non-GeoDataFrame if this function is mistakenly used
        # or if a function might receive either DataFrame or GeoDataFrame
        if isinstance(gdf, pd.DataFrame):
            try:
                return gdf.to_parquet()
            except Exception: # pragma: no cover
                return str(gdf) # Final fallback
        return gdf 

    try:
        # Hash based on WKT of geometry and Parquet of other data
        # Using a tuple of hashes for individual components might be more robust
        # to changes in serialization order if not using a canonical format like Parquet.
        
        # Ensure geometry column exists and is valid before trying to get WKT
        geom_col_name = gdf.geometry.name
        if geom_col_name not in gdf.columns or gdf[geom_col_name].empty:
            geometry_hash_part = b"" # Empty geometries, empty hash part
        else:
            # Handle potential empty geometries within the column
        ...
        # Hash the rest of the DataFrame's data (as bytes using Parquet)
        # Ensure a consistent column order for the non-geometry part before hashing
        # Sort columns alphabetically to ensure hash consistency if column order changes
        ...
        logger.error(f"Error generating hash for GeoDataFrame (falling back to string): {e}", exc_info=True)
        # Fallback: hash the string representation. Less ideal, can be slow, but better than unhashable.
        # This might still fail if str(gdf) is too large or complex.
        return str(gdf) # pragma: no cover


# --- Data Loading Functions ---
@st.cache_data(ttl=3600)
def load_health_records():
    # ... (content as provided in the previous "complete corrected file")
...
def load_zone_data():
    ...
    except pd.errors.EmptyDataError as ede: # pragma: no cover
...
@st.cache_data(ttl=3600, hash_funcs={gpd.GeoDataFrame: hash_geodataframe, pd.DataFrame: lambda df: df.to_parquet()})
def enrich_zone_geodata_with_health_aggregates(zone_gdf, health_df):
    # ... (content as provided in the previous "complete corrected file", ensure hash_funcs is applied)
    # ... (Make sure any internal dataframes created and not returned are not an issue for hashing if they were gdf)
    if health_df.empty:
        ...
        # Create a copy to modify if zone_gdf is not None
        zone_gdf_enriched = zone_gdf.copy() if zone_gdf is not None else gpd.GeoDataFrame()
        for col in default_cols:
            if col not in zone_gdf_enriched.columns: zone_gdf_enriched[col] = 0
...
# --- KPI Calculation Functions (ensure they also have hash_funcs if they take GDFs/DFs and are cached) ---
@st.cache_data(hash_funcs={pd.DataFrame: lambda df: df.to_parquet()})
def get_overall_kpis(df_health_records):
    ...
    positive_tests = df_copy[
        (df_copy['test_result'].astype(str) == 'Positive') & (df_copy['test_date'].notna()) & (df_copy['test_date'] >= one_week_ago) # Added notna check for test_date
...
def get_chw_summary(df_chw_view):
    ...
    high_risk_followups = df_copy[
        ...
        (~df_copy['referral_status'].isin(['Completed', 'Closed', 'Unknown']))
    ].shape[0]
...
def get_patient_alerts_for_chw(df_chw_view, risk_threshold=None):
    ...
    def determine_reason_chw(row): # Ensure row elements are checked for NaNs if they can occur
...
def get_clinic_summary(df_clinic_view):
    ...
    critical_supply = df_copy[
        ...
        (~df_copy['supply_item'].isin(['Unknown', 'N/A', 'nan'])) # Exclude unknown items
    ]['supply_item'].nunique()
...
def get_patient_alerts_for_clinic(df_clinic_view, risk_threshold=None):
    # ... (content as provided in the previous fix, including the 'row['condition'] in [...]' correction)
    if df_clinic_view.empty: return pd.DataFrame()

    df_copy = df_clinic_view.copy()
    df_copy['date'] = pd.to_datetime(df_copy['date'], errors='coerce')
    df_copy['test_date'] = pd.to_datetime(df_copy['test_date'], errors='coerce')
    df_copy.dropna(subset=['date'], inplace=True) # Critical for latest_view_date
    if df_copy.empty: return pd.DataFrame()

    df_copy['ai_risk_score'] = pd.to_numeric(df_copy['ai_risk_score'], errors='coerce')
    for col in ['test_result', 'condition', 'referral_status', 'patient_id', 'zone_id']:
         if col in df_copy.columns: # Ensure column exists before astype
            df_copy[col] = df_copy[col].astype(str).fillna('Unknown')

    risk_thresh = risk_threshold if risk_threshold is not None else app_config.RISK_THRESHOLDS['moderate']
    latest_view_date = df_copy['date'].max() # Max of existing dates
    recent_test_cutoff = latest_view_date - pd.Timedelta(days=7)

    # Build conditions ensuring NaNs in ai_risk_score are handled (they won't satisfy >= risk_thresh)
    # and test_date is not NaT before comparison
    cond1 = df_copy['ai_risk_score'].notna() & (df_copy['ai_risk_score'] >= risk_thresh)
    cond2 = (df_copy['test_result'] == 'Positive') & df_copy['test_date'].notna() & (df_copy['test_date'] >= recent_test_cutoff)
    cond3 = (df_copy['test_result'] == 'Pending') & (df_copy['condition'].isin(['TB','Malaria']))
    
    alert_conditions = cond1 | cond2 | cond3
    alerts_df = df_copy[alert_conditions].copy()
    ...
    def determine_reason_clinic(row):
        ...
        if row['test_result'] == 'Pending' and row['condition'] in ['TB','Malaria']:
    ...
    alerts_df = alerts_df.sort_values(by=['ai_risk_score', 'date'], ascending=[False, False]) # Sort by risk then date
    alerts_df.drop_duplicates(subset=['patient_id'], keep='first', inplace=True)
