...
def get_patient_alerts_for_clinic(df_clinic_period_view, risk_threshold_moderate=None):
    ...
    latest_records_in_period.loc[:, 'cond_clinic_hiv_high_vl'] = ((latest_records_in_period.get('condition', s_false.copy()) == 'HIV-Positive') & (latest_records_in_period.get('hiv_viral_load', pd.Series(dtype=float)).notna()) & (latest_records_in_period.get('hiv_viral_load', pd.Series(dtype=float)) > 1000) )
    ...
    def determine_clinic_alert_reason_and_priority(row): 
        ...
        if row.get('cond_clinic_hiv_high_vl', False): reasons.append(f"High HIV VL ({row['hiv_viral_load']:.0f})"); priority_score += 100
        if row.get('cond_clinic_high_risk', False): reasons.append(f"High Risk ({row['ai_risk_score']:.0f})"); priority_score += row.get('ai_risk_score', 0)
