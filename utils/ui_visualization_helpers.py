...
try:
    ...
    else: # Handle cases where token is present but invalid or placeholder
...
def _get_theme_color(index=0, fallback_color="#007bff"):
    """Safely retrieves a color from the custom theme's colorway or Plotly's default."""
    try:
        ...
        if custom_template and hasattr(custom_template, 'layout') and \
           hasattr(custom_template.layout, 'colorway') and custom_template.layout.colorway:
            return custom_template.layout.colorway[index % len(custom_template.layout.colorway)]
        
        # Try Plotly's base "plotly" theme if custom one has no colorway (shouldn't happen with current setup)
        ...
        if plotly_default_template and hasattr(plotly_default_template, 'layout') and \
           hasattr(plotly_default_template.layout, 'colorway') and plotly_default_template.layout.colorway:
            ...
            return plotly_default_template.layout.colorway[index % len(plotly_default_template.layout.colorway)]
            
    except Exception as e_color_get:
        logger.warning(f"Could not retrieve theme color at index {index}: {e_color_get}. Using fallback: {fallback_color}")
...
def set_custom_plotly_theme():
    ...
    custom_theme.layout.title = dict(font=dict(size=18), x=0.02, xanchor='left', y=0.97, yanchor='top', pad=dict(t=20, b=10)) # Increased top padding for title
    ...
    custom_theme.layout.margin = dict(l=70, r=30, t=90, b=70) # Increased top margin for legend and title
...
def render_kpi_card(title, value, icon, status="neutral", delta=None, delta_type="neutral", help_text=None, icon_is_html=False):
    ...
    semantic_status_class = "status-" + status.lower() if status and status.lower() in ["good", "bad"] else ""
...
def plot_layered_choropleth_map(gdf: gpd.GeoDataFrame, value_col: str, title: str, id_col: str = 'zone_id', featureidkey_prefix: str = 'properties', color_continuous_scale: str = "Blues_r", hover_cols: list = None, facility_gdf: gpd.GeoDataFrame = None, facility_size_col: str = None, facility_hover_name: str = None, facility_color: str = None, height: int = None, center_lat: float = None, center_lon: float = None, zoom_level: int = None, mapbox_style: str = None):
    ...
    current_mapbox_style = mapbox_style if mapbox_style else _get_theme_color('mapbox_style', app_config.MAPBOX_STYLE) # _get_theme_color adapted
    ...
    if facility_gdf is not None and not facility_gdf.empty and 'geometry' in facility_gdf.columns:
        ...
        if not facility_plot_gdf.empty:
            ...
            final_facility_color = facility_color if facility_color else _get_theme_color(5) # Use a theme color for facilities
...
def plot_annotated_line_chart(data_series: pd.Series, title: str, y_axis_title: str = "Value", color: str = None, target_line: float = None, target_label: str = None, show_ci: bool = False, lower_bound_series: pd.Series = None, upper_bound_series: pd.Series = None, height: int = None, show_anomalies: bool = True, date_format: str = "%b %Y"):
    ...
    fig = go.Figure()
    line_color = color if color else _get_theme_color(0)
    fig.add_trace(go.Scatter(x=data_series.index, y=data_series.values, mode="lines+markers", name=y_axis_title, line=dict(color=line_color, width=2.8), marker=dict(size=7, symbol='circle'), customdata=data_series.values, hovertemplate=(f'<b>Date</b>: %{{x|{date_format}}}<br><b>{y_axis_title}</b>: %{{customdata:,.2f}}<extra></extra>')))
    if show_ci and lower_bound_series is not None and upper_bound_series is not None and not lower_bound_series.empty and not upper_bound_series.empty:
        ...
        if not common_idx_ci.empty:
            ...
            if valid_ci_mask.any():
                ...
                fill_color_rgba = f"rgba({','.join(str(int(c, 16)) for c in (line_color[1:3], line_color[3:5], line_color[5:7]))},0.18)" if line_color.startswith('#') and len(line_color) == 7 else "rgba(0,123,255,0.18)"
...
def plot_donut_chart(data_df_input, labels_col: str, values_col: str, title: str, height: int = None, color_discrete_map: dict = None, pull_segments: float = 0.03, center_text: str = None):
    ...
    plot_colors_donut = None
    if color_discrete_map: plot_colors_donut = [color_discrete_map.get(lbl, _get_theme_color(i % len(app_config.DISEASE_COLORS if hasattr(app_config,"DISEASE_COLORS") else [_get_theme_color(j) for j in range(5)] ))) for i, lbl in enumerate(df[labels_col])] # Robust color access
    elif hasattr(app_config,"DISEASE_COLORS") and app_config.DISEASE_COLORS: # Fallback to app_config.DISEASE_COLORS if no map
         plot_colors_donut = [app_config.DISEASE_COLORS.get(lbl, _get_theme_color(i % len(app_config.DISEASE_COLORS))) for i,lbl in enumerate(df[labels_col])]


    fig = go.Figure(data=[go.Pie(labels=df[labels_col], values=df[values_col], hole=0.52, pull=[pull_segments if i < 3 else 0 for i in range(len(df))], textinfo='label+percent', hoverinfo='label+value+percent', insidetextorientation='radial', marker=dict(colors=plot_colors_donut, line=dict(color='#ffffff', width=2.2)), sort=False)])
...
def plot_heatmap(matrix_df_input, title: str, height: int = None, colorscale: str = "RdBu_r", zmid: float = 0, text_auto: bool = True, text_format: str = ".2f", show_colorbar: bool = True):
    ...
    df_plot_heatmap = df_numeric.fillna(0) # Or use np.nan and handle hovertemplate for NaNs
    z_vals = df_plot_heatmap.values; text_vals_heatmap = np.around(z_vals, decimals=int(text_format[-2]) if text_format.endswith('f') and text_format[-2].isdigit() else 2)
    zmid_final = zmid if pd.Series(z_vals.flatten()).min() < 0 and pd.Series(z_vals.flatten()).max() > 0 else None # Only if data crosses zero
    fig = go.Figure(data=go.Heatmap(z=z_vals, x=df_plot_heatmap.columns.tolist(), y=df_plot_heatmap.index.tolist(), colorscale=colorscale, zmid=zmid_final, text=text_vals_heatmap if text_auto else None, texttemplate=f"%{{text:{text_format}}}" if text_auto else None, hoverongaps=False, xgap=1.8, ygap=1.8, colorbar=dict(thickness=20, len=0.9, tickfont_size=10, title_side="right", outlinewidth=1, outlinecolor=_get_theme_color(0,"#ced4da")) if show_colorbar else None ))
    rotate_x = -40 if len(df_plot_heatmap.columns) > 8 or max(len(str(c)) for c in df_plot_heatmap.columns if c is not None) > 10 else 0
